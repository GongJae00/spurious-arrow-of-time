"""Generate manuscript Figure 3, the benchmark-construction example."""
...
def save_figure(fig: plt.Figure, out_dir: Path, name: str) -> None:
    ...
    fig.savefig(figure_dir / f"{name}.png", bbox_inches="tight", pad_inches=0.02, dpi=400)
    plt.close(fig)
...
def figure_benchmark(out_dir: Path, config_path: Path) -> None:
    ...
    save_figure(fig, out_dir, "fig3_benchmark_construction")


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", type=Path, default=Path("configs/irreversible_source_extended.yaml"))
