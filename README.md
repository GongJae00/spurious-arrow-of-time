...
The method is a six-gate cue-locality audit on factor-intervenable temporal benchmarks: Gate 1 core accessible, Gate 2 nuisance predictive, Gate 3 endpoint controlled, Gate 4 core recoverable, Gate 5 reversal attribution (OOD reverses P(ds|y)), Gate 6 cue locality (single-frame, unordered set, shuffle/reverse → frame-local, order-invariant multi-frame, or order-encoded). The generator makes each gate independently testable.
...
`GeneratorConfig` (Algorithm 2) → named construction (`make`) → CNN+GRU ERM → `Audit.gate1`–`gate6` (Algorithm 1) → table JSON / figure.

| Path | Paper object |
|---|---|
| `src/data.py` | Algorithm 2: core diffusion (Eq. 6), directional nuisance (Eqs. 7–8), two-channel observation (Eq. 9) |
...
| `src/audit.py` | `Audit.gate1`–`gate6` (Algorithm 1). Gate 5 is OOD reversal; Gate 6 locates the cue. |
